...
ExJW6()
{
	...
	getContentPane().add(B6); getContentPane().add(T2);
		...
		{
			if(e.getSource()==B1)
			L1.setText("Texto: " + combo.getSelectedItem()); //obtém o texto do item

			if(e.getSource()==B2)
			L1.setText("Índice: "+ combo.getSelectedIndex()); // obtém o índice do item
			...
			if(e.getSource()==B4)
			combo.removeItemAt(combo.getSelectedIndex()); // remove o item conforme o índice

			if(e.getSource()==B5)
			combo.removeAllItems(); // remove todos os itens da lista

			if(e.getSource()==B6)
			T2.setText(" " + combo.getItemCount()); //obtém a quantidade total de itens
